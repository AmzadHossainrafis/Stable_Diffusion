...
def train_clip(model, dataloader, optimizer, device, epochs=50):
    ...
    for epoch in range(epochs):
        ...
        for i, (images, input_ids, attention_mask) in pbar:
            ...
            pbar.set_description(f"Epoch {epoch+1}, Loss: {torch.tensor(losses).mean():.4f}")
            optimizer.step()
...
class CLIPTrainer:
    ...
    def save_model(self, epoch:int , interval:int): 
        ...
        if epoch % interval == 0:
            # Save the model
            torch.save(self.model.state_dict(), os.path.join(self.config['artifact'],"clip_model.pth"))
            print("Model saved successfully.")
            #text encoder 
            torch.save(self.model.text_encoder.state_dict(), os.path.join(self.config['artifact'],"clip_text_encoder.pth"))
            print("Text encoder saved successfully.")
            #image encoder
            torch.save(self.model.image_encoder.state_dict(), os.path.join(self.config['artifact'],"clip_image_encoder.pth"))
